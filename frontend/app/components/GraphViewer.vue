  ...
  <section v-if="nodeCount" class="network-shell">
    <aside class="network-sidebar">
      ...
      <section class="sidebar-block controls">
        <h3>Controls</h3>
        <label>
          <span>Link distance <strong>{{ linkDistance }}</strong></span>
          <input v-model.number="linkDistance" type="range" min="60" max="300" step="10" />
        </label>
        <label>
          <span>Charge strength <strong>{{ chargeStrength }}</strong></span>
          <input v-model.number="chargeStrength" type="range" min="-1400" max="-100" step="50" />
        </label>
        <label class="check">
        ...
        <div class="control-actions">
          <button type="button" @click="fitGraph">Reset zoom</button>
          <button type="button" @click="restartLayout">Relayout</button>
        </div>
      </section>
    </aside>
...
const activeHoverNode = computed(() => {
  ...
  return visibleNodes.value[props.hoveredNodeKey] || props.nodes[props.hoveredNodeKey] || null
})
...
function createConfigs() {
  return vNG.defineConfigs<GraphNode>({
    view: {
      ...
      layoutHandler: new ForceLayout({
        ...
        createSimulation: (d3, nodes, edges) => {
          ...
          return d3
            ...
            .force("collide", d3.forceCollide().radius(28).strength(0.8))
    ...
    node: {
      selectable: true,
      normal: {
        radius: node => 12 + Math.min((node.degree || 0) * 3.5, 22),
        color: node => nodeColor(node.type),
        strokeWidth: 2,
        strokeColor: "#071526",
      },
      hover: {
        radius: node => 15 + Math.min((node.degree || 0) * 3.5, 22),
        strokeWidth: 3,
        strokeColor: "#d9fcff",
      },
      selected: {
        radius: node => 16 + Math.min((node.degree || 0) * 3.5, 22),
        color: node => nodeColor(node.type),
        strokeWidth: 4,
        strokeColor: "#f8fafc",
      },
...
const configs = ref(createConfigs())
...
function restartLayout() {
  configs.value = createConfigs()
  layoutRevision.value += 1
}

watch([linkDistance, chargeStrength, showLabels, showEdgeLabels], restartLayout)
...
.network-shell {
  ...
  overflow: hidden;
  background: #050d1b;
  box-shadow: 0 24px 58px rgba(2, 6, 23, .3);
}
.network-sidebar {
  min-height: 710px;
  display: flex;
...
.node-list { min-height: 90px; max-height: 240px; overflow: auto; padding-right: 3px; }
.node-item {
  display: flex;
  align-items: start;
  gap: 10px;
...
.controls label { display: block; font-size: .76rem; color: #8fa5bf; margin: 9px 0; }
.controls label span { display: flex; justify-content: space-between; margin-bottom: 5px; }
...
.control-actions button {
  flex: 1; padding: 8px; color: #bfeafd; border: 1px solid #203958;
  background: #0b192c; border-radius: 8px; cursor: pointer;
}
.network-canvas {
  position: relative;
  min-width: 0;
  background:
    radial-gradient(circle at 48% 44%, rgba(34, 211, 238, .06), transparent 34%),
    #050d1b;
}
.canvas-header {
  position: absolute; z-index: 2; top: 0; left: 0; right: 0;
  display: flex; justify-content: space-between; align-items: baseline;
  height: 55px; padding: 16px 21px 0;
  pointer-events: none;
}
...
.tooltip-type {
  display: block; margin: 5px 0 10px; font-size: .74rem;
...
@media (max-width: 980px) {
  .network-shell { display: block; }
  .network-sidebar { min-height: auto; }
  .node-list { max-height: 180px; }
  .graph { height: 600px; }
}
</style>
