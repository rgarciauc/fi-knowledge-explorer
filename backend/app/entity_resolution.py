...
STOPWORDS = {
    ...
    "the", "to", "what", "when", "which", "who", "with", "would", "team",
    "department",
}
...
def _candidate_score(search_text: str, row: dict) -> float:
    ...
    if not name:
        return 0.0
    if name in search or (node_id and node_id in search):
        return 1.0
    search_tokens = meaningful_tokens(search)
    candidate_tokens = meaningful_tokens(f"{name} {node_id}")
    token_score = max((_ratio(left, right) for left in search_tokens for right in candidate_tokens), default=0.0)
    phrase_score = max(_ratio(search, name), _ratio(search, node_id))
    description_score = max((_ratio(token, word) for token in search_tokens for word in meaningful_tokens(description)), default=0.0)
    return min(1.0, max(token_score, phrase_score, description_score * 0.84))

def find_entity_candidates(question: str, catalog: list[dict], *, labels: set[str] | None = None, term: str | None = None, limit: int = 7) -> list[EntityCandidate]:
...
def best_candidate_for_intent(intent: str, candidates: list[EntityCandidate]) -> EntityCandidate | None:
    label_by_intent = {
        ...
        "system_owners": {"System"},
        "ownership_search": {"System", "Team", "Employee"},
        "process_pipeline": {"BusinessProcess"},
        "next_step": {"ProcessStep"},
        "employee_search": {"Employee"},
        "employee_responsibilities": {"Employee", "Responsibility"},
        "department_employees": {"Department"},
        "data_lineage": {"System", "DataPipeline", "Dataset", "ExternalSource"},
        "regulatory_oversight": {"RegulatoryFramework", "Team", "System"},
    }
